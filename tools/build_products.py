...
Produces:
  ...
  src/partials/acrylic-grid.html     cards for acrylic-tanks.html
  src/partials/acrylic-ranges.html   its "shop by" tiles, and acrylic-chips.html its filters
  product/<slug>.html                one page per acrylic product
    (all from data/acrylic-products.json -- Advanced Acrylics' catalogue, pulled by
     tools/fetch_acrylic.py -- or, until that exists, data/acrylic-tanks.json styles)
...
# Product pages live one level down (product/), so every root-relative path shifts.
def reroot(markup):
...
def product_page(meta, body, schema, source):
    """A full product/<slug>.html: the shared chrome around one page body."""
    if not _TEMPLATES:
        for key, name in (("head", "page-head.html"), ("header", "page-header.html"),
                          ("footer", "home-footer.html")):
            _TEMPLATES[key] = read(os.path.join(PARTIALS, name))
    head, header = _TEMPLATES["head"], _TEMPLATES["header"]
    ...
    return (
        "<!-- Generated by tools/build_products.py from %s.\n"
        "     Edit the JSON and regenerate; do not edit this file. -->\n" % source
        + head + "\n" + header + body + reroot(_TEMPLATES["footer"])
...
ACRYLIC_PRODUCTS = os.path.join(ROOT, "data", "acrylic-products.json")


def build_acrylic_catalogue(entry_for):
    """acrylic-tanks.html from Advanced Acrylics' catalogue (client, 22 Sept:
    "copy all his pictures ... put in the shop acrylic aquariums ... Arrange
    different"), written by tools/fetch_acrylic.py. Same card, tile and product
    page shapes as the glass shop; the grouping and order are ours
    (fetch_acrylic.GROUPS). No cart: every call to action is the quote form,
    ...
    def price_text(it):
        ...
        if it["price_from"] != it["price_to"]:
            return "From $%s" % it["price_from"]
        return "$%s" % it["price_from"]

    def chips(it):
        n = len(it["variants"])
        return ["%d options" % n] if n > 1 else []

    def media(it, sizes, prefix="", priority=False):
        img = it["images"][0] if it["images"] else None
        pic = picture(entry_for(img), img, it["name"], sizes, prefix=prefix, priority=priority) if img else ""
        return pic, ("" if pic else " -empty")

    def card(it, prefix="", link_prefix="product/"):
        pic, empty = media(it, "(min-width: 1025px) 22vw, (min-width: 641px) 45vw, 90vw", prefix)
        return (
            '<article class="m-tankCard" data-tank data-range="%s" data-price="%.2f" data-gallons="0" data-name="%s">\n'
            '  <a class="m-tankCard__link" href="%s%s.html">\n'
            '    <div class="m-tankCard__media%s">%s</div>\n'
            '    <div class="m-tankCard__body">\n'
            '      <p class="m-tankCard__range">%s</p>\n'
            '      <h3 class="m-tankCard__name">%s</h3>\n'
            '      <ul class="m-tankCard__chips">%s</ul>\n'
            '      <div class="m-tankCard__foot"><span class="m-tankCard__price">%s</span>'
            '<span class="m-tankCard__cta">View %s</span></div>\n'
            '    </div>\n'
            '  </a>\n'
            '</article>' % (it["group"], float(it["price_from"].replace(",", "") or 0),
                            esc(it["name"].lower()), link_prefix, it["slug"], empty, pic,
                            esc(labels[it["group"]]), esc(it["name"]),
                            "".join("<li>%s</li>" % esc(c) for c in chips(it)),
                            esc(price_text(it)), ARROW))

    head = ("<!-- Generated by tools/build_products.py from data/acrylic-products.json.\n"
            "     Edit via tools/fetch_acrylic.py, not this file. -->\n")
    changed = write_if_changed(os.path.join(PARTIALS, "acrylic-grid.html"),
    ...
    for key, label in GROUPS:
        ...
        if not group:
            continue
        lead = next((it for it in group if it["images"]), group[0])
        pic, _ = media(lead, "(min-width: 1025px) 11vw, 40vw")
        tiles.append(
            ...
            '<span class="m-rangeTile__count">%d product%s</span></button></li>'
            % (key, pic, esc(label), len(group), "" if len(group) == 1 else "s"))
    changed += write_if_changed(os.path.join(PARTIALS, "acrylic-ranges.html"), head + "\n".join(tiles) + "\n")
    chips_html = "\n".join(
        '        <button class="o-shop__chip" type="button" data-shop-range="%s" aria-pressed="false">%s</button>'
        % (key, esc(label)) for key, label in GROUPS if any(it["group"] == key for it in items))
    changed += write_if_changed(os.path.join(PARTIALS, "acrylic-chips.html"), head + chips_html + "\n")

    # ---- one page per product ------------------------------------------------
    written = 0
    for it in items:
        name, slug, label = it["name"], it["slug"], labels[it["group"]]
        # quote_plus, not a bare space->+ swap: these names carry "#" ("REF# FT108"),
        # which would otherwise end the query string and cut the name to "REF".
        quote = "../quote.html?kind=acrylic&amp;tank=%s" % esc(urllib.parse.quote_plus(name))
        slides, thumbs = [], []
        for n, img in enumerate(it["images"]):
            entry = entry_for(img)
            slides.append('<li class="m-gallery__slide" id="%s-%d">%s</li>' % (
                slug[:40], n + 1,
                picture(entry, img, "%s, photo %d" % (name, n + 1) if n else name,
                        "(min-width: 1025px) 45vw, 92vw", prefix="../", priority=(n == 0),
                        loading="lazy")))
            if len(it["images"]) > 1:
                thumbs.append('<li><button class="m-gallery__thumb%s" type="button" data-gallery-to="%d" '
                              'aria-label="Photo %d of %d">%s</button></li>' % (
                                  " -active" if n == 0 else "", n, n + 1, len(it["images"]),
                                  picture(entry, img, "", "8rem", prefix="../")))
        gallery = ('<div class="m-gallery" data-gallery><ul class="m-gallery__stage" data-gallery-stage>%s</ul>%s</div>'
                   % ("".join(slides), '<ul class="m-gallery__thumbs">%s</ul>' % "".join(thumbs) if thumbs else "")
                   if slides else '<div class="m-tankCard__media -empty -large"></div>')
        table = ('<table class="o-tank__table"><thead><tr><th scope="col">Option</th><th scope="col">Price</th></tr></thead>'
                 '<tbody>%s</tbody></table>' % "".join(
                     '<tr><th scope="row">%s</th><td>$%s</td></tr>' % (esc(v["title"]), esc(v["price"]))
                     for v in it["variants"])) if it["variants"] else ""
        desc = ('<ul class="o-tank__list">%s</ul>' % "".join("<li>%s</li>" % esc(l) for l in it["description"])
                if it["description"] else "")
        details = []
        if table:
            details.append(("Options &amp; prices", table, True))
        if desc:
            details.append(("Details", desc, not table))
        acc = "\n".join(
            ...
            for t, b, op in details)
        related = [q for q in items if q["group"] == it["group"] and q is not it][:4]
        body = '''<section class="m-pageBanner -compact" data-page-hero>
  <div class="m-pageBanner__media" aria-hidden="true">
    <picture><source type="image/webp" srcset="../assets/images/featured/featured-acrylic-tank-bench-600.webp 600w, ../assets/images/featured/featured-acrylic-tank-bench-1000.webp 1000w, ../assets/images/featured/featured-acrylic-tank-bench-1284.webp 1284w" sizes="100vw"><img src="../assets/images/featured/featured-acrylic-tank-bench.jpg" alt="" width="1284" height="956" decoding="async"></picture>
  </div>
  <div class="m-pageBanner__inner">
    <nav class="m-pageBanner__crumbs" aria-label="Breadcrumb"><ol><li><a href="../index.html">Home</a></li><li><a href="../acrylic-tanks.html">Acrylic Tanks</a></li><li aria-current="page">%(name)s</li></ol></nav>
...
<section class="o-tank" aria-label="%(name)s">
  <div class="row">
    <div class="o-tank__grid">
      <div class="o-tank__media -gallery">%(gallery)s</div>
...
%(related)s''' % {
            "name": esc(name), "label": esc(label), "gallery": gallery, "price": esc(price_text(it)),
            "chips": "".join("<li>%s</li>" % esc(c) for c in chips(it)), "quote": quote,
            "arrow": ARROW, "acc": acc,
            "related": ('''
<section class="o-shopRelated" aria-labelledby="related-title">
  <div class="row">
    <div class="o-shopRelated__head">
      <h2 class="tx-h3" id="related-title">More <strong>%s</strong></h2>
      <a class="a-link tx-cta" href="../acrylic-tanks.html">Back to all acrylic tanks</a>
    </div>
...
''' % (esc(label.lower()), "\n".join(card(q, prefix="../", link_prefix="") for q in related))) if related else ""}

        summary = " ".join(it["description"])[:300]
        description = ("%s. %s from Acrylic Pros, %s.%s" % (
            name, label, price_text(it).lower(), (" " + summary) if summary else ""))[:300]
        meta = {
            "TITLE": "%s | Acrylic Tanks — Acrylic Pros" % esc(name),
            "DESCRIPTION": esc(description), "OG_TITLE": esc(name), "OG_DESCRIPTION": esc(description),
            "OG_TYPE": "product",
            "OG_IMAGE": it["images"][0] if it["images"] else "assets/images/featured/featured-acrylic-tank-bench.jpg",
            "OG_IMAGE_ALT": esc(name), "CANONICAL": "product/%s.html" % slug,
            "BODY_CLASS": "t-inner t-shop t-product t-acrylic", "NAV": "acrylic-tanks",
            "SITE_URL": SITE_URL, "PRELOAD": "",
        }
        schema = {
            "@context": "https://schema.org", "@type": "Product", "name": name, "sku": slug,
            "category": "Acrylic Tanks / %s" % label,
            "image": [SITE_URL + i for i in it["images"]],
            "description": " ".join(it["description"])[:500],
            "url": SITE_URL + "product/%s.html" % slug,
        }
        if it["price_from"]:
            schema["offers"] = {
                "@type": "AggregateOffer", "priceCurrency": "USD",
                "lowPrice": it["price_from"].replace(",", ""), "highPrice": it["price_to"].replace(",", ""),
                "offerCount": max(1, len(it["variants"])), "availability": "https://schema.org/InStock",
                "url": SITE_URL + "product/%s.html" % slug,
            }
...
def build_acrylic(entry_for):
    ...
    chips = "\n".join(
        '        <button class="o-shop__chip" type="button" data-shop-range="%s" aria-pressed="false">%s</button>'
        % (key, label) for key, label in ACRYLIC_RANGES)
    return (write_if_changed(os.path.join(PARTIALS, "acrylic-grid.html"), head + "\n".join(cards) + "\n")
            ...
            + write_if_changed(os.path.join(PARTIALS, "acrylic-chips.html"), head + chips + "\n"))
...
def main():
    ...
    changed += write_if_changed(os.path.join(PARTIALS, "product-compare.html"),
                                "<!-- Generated by tools/build_products.py -->\n"
                                + "\n".join(rows) + "\n")
    # Advanced Acrylics' catalogue once tools/fetch_acrylic.py has run; until
    # then the quote-only styles from data/acrylic-tanks.json.
    changed += (build_acrylic_catalogue(entry_for) if os.path.isfile(ACRYLIC_PRODUCTS)
                else build_acrylic(entry_for))
...
''' % {"banner": banner, "name": esc(name), "label": esc(label), "media": media_block,
       ...
       "related": related_cards}
        ...
        written += write_if_changed(os.path.join(OUT_DIR, slug + ".html"),
                                    product_page(meta, body, schema, "data/products.json"))
